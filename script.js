...
let imgShadow =
  "rgba(0, 0, 0, 0.19) 0px 10px 20px, rgba(0, 0, 0, 0.2) 0px 6px 6px";
let imgSize = "350";
let imgSrc = "ico/retg";
...
function darkAquarium() {
  ...
  imgShadow =
    "rgba(0, 0, 20, 0.25) 0px 2px 5px , rgba(0, 0, 0, 0.3) 0px 1px 3px ";
  imgSize = "320";
  imgSrc = "ico/retg.png";
...
function writePrompt() {
  ...
  getSearch();
  prompt.innerHTML = `
    <div class='prompt-title'><span class="prePrompt" id="prePrompt">${prePrompt}</span><span class="atSymb" id="atSymb">@</span><span class="promptSearch" id="searchType">${searchType}</span>${
    position.length ? "/" : ""
  }${position.join(
    "/"
...
// Defined Themes
const THEMES = ["dark", "light", "nord", "catp", "tokyo"];
...
function changeEngine(searchEng) {
  ...
  if (searchEng) {
    currentSearchUrl = ENGINES[searchEng][0]
      ? ENGINES[searchEng][0]
      : ENGINES.google[0];
...
/* Global declarations */
let searchUrl =
  ENGINES.google[0]; /* default search engine -- CHANGE this value */
